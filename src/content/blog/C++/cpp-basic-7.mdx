...
:::tip
从这一章开始，你的代码就不再是一潭死水的数据了。你要开始创造“活生生”的对象了。
:::
...
```
看懂了吗？
函数 `heal` 变成了 `Player` 的一部分！
调用的时候，我们写的是 `p1.heal(20)`。
...
现实中，你用一台自动售货机。
你能接触到的，是按键和投币口（这是 `public`，公开的）。
你接触不到的，是机器内部的钱箱和电路板（这是 `private`，私有的）。

如果你能直接伸手进钱箱改余额，世界就乱套了。写代码也是一样。
```cpp
#include <iostream>
#include <string>
using namespace std;
...
答案是：能！ 在 C++ 里，`struct` 几乎能干 `class` 能干的所有事情。

那它们俩到底有啥区别？
其实只有一点小小的区别：默认的可见性不同。

如果什么都不写：
`struct` 里的东西默认是 `public`（公开的）。
`class` 里的东西默认是 `private`（私有的）。
```cpp
struct A {
    int x; // 默认是 public
};
...
在实际开发中的潜规则是：
- 如果这个东西主要就是为了装一堆公开数据（比如一个坐标、一个配置项），用 `struct`。
- 如果这个东西有复杂的操作逻辑，需要隐藏数据保护状态，用 `class`。
...
在这个例子里，怪兽不是被动的数据，它们是“活”的。

它们有自己的名字、有自己的逻辑（血量不能小于 0），甚至还能互动（调用对方的方法）。
