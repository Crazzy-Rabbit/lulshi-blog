...
:::note
以后你见到绝大多数拷贝构造函数，基本都是这个样子。它不是凑巧，而是最合理、最通用的写法。
:::
...
![拷贝构造触发时机](/images/cpp-basic/ch08/copy-constructor-flow.svg)
...
![浅拷贝与深拷贝对比图](/images/cpp-basic/ch08/shallow-vs-deep-copy.svg)
...
- `+`、`==`、`[]` 这些运算符能不能让类自己支持
- 为什么 `cout << obj` 这种写法背后也和运算符重载有关
- 什么是 `friend`，它为什么能让某些外部函数访问私有成员
- 什么时候应该重载，什么时候不该乱重载
