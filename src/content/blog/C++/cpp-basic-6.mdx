...
前面我们学过变量、函数、数组、字符串、引用和指针。到这里，你会慢慢发现一个问题。

现实世界里的数据，往往不是单独一个 `int`、一个 `double`、一个 `string` 就能说清楚的。

比如一个学生，至少会有姓名、年龄、成绩。比如一本书，至少会有书名、作者、价格。比如游戏里的角色，至少会有名字、血量、攻击力和坐标。

这时候，如果还把这些信息拆成一堆零散变量，代码就会越来越乱。于是 C++ 说，别急，我们可以自己“造类型”。
...
:::tip
从这一章开始，你的思维要慢慢从“我有几个变量”转成“我在描述一个对象”。
:::
...
## struct 和 class 现在先怎么理解

你后面一定会学到 `class`。很多人到这里都会问，那 `struct` 和 `class` 到底啥关系。
...
在你当前阶段，可以先把 `struct` 看成更适合入门的数据打包工具。
...
这不会创造一个全新的底层类型，它只是给原来的类型起了一个别名。
...
:::tip
当一个变量只允许在几个固定状态里变化时，枚举往往比裸数字更优雅。
:::
...
到这里，这一章最重要的不是你会不会把例子一字不差背下来，而是你要开始建立一种新的程序视角。

第一，当一组数据属于同一个对象时，用 `struct` 把它们装起来。

第二，当类型名字太长，或者你想让代码含义更清楚时，用 `using` 起别名。

第三，当一个变量只能取若干固定状态时，用 `enum` 表示会更自然。

第四，结构体会和数组、函数、条件判断一起配合，它不是孤立知识点。
...
如果说前几章你还在学怎么和 C++ 打招呼，那么从这一章开始，你已经在认真地和它一起搭世界了。

一个学生、一位玩家、一本书、一个坐标点，这些原本只是脑海里的概念，现在都能被你用代码正式定义出来。

这很像你突然拿到了一支更高级的画笔。

之前你只能画点和线。

现在，你开始能画角色卡、物品栏、地图坐标和规则系统了。

欢迎进入更像“编程”的编程阶段。

下一章很适合继续进入 C++ 的面向对象初步，比如类与对象。
